...
q3 = '''select * from (
      select t1.day,
      round(cast((100*t2.num) as numeric) / cast(t1.num as numeric), 2) as err
      from (select cast(time as date) as day, count(*) as num
      from log group by day) as t1
      inner join
      (select cast(time as date) as day, count(*) as num from log
      where status='404 NOT FOUND' group by day) as t2
      on t1.day=t2.day)
